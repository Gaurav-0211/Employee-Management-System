...
// Base UI Components (kept inside main page)

// Section Components
import DangerZoneSection from "../components/settingComponents/DangerZoneSection";
import PreferencesSection from "../components/settingComponents/PreferencesSection";
import PrivacySection from "../components/settingComponents/PrivacySection";
...
export default function EmployeeSettingsPage() {
    ...
    try {
      ...
      for (const url of tryUrls) {
        try {
          ...
          if (!res.ok) continue;
          const parsed = await res.json();
          json = parsed.data ? parsed.data : parsed;
          break;
        } catch {}
      }
      ...
      setProfile({
        ...
        email: json.email || "",
        contactNumber: json.contactNumber || json.contact || "",
      ...
      if (json.preferences) {
        setNotifications(!!json.preferences.notifications);
        setTwoFactor(!!json.preferences.twoFactor);
        setProfileVisibility(!!json.preferences.profileVisibility);
        setDataSharing(!!json.preferences.dataSharing);
        setThemeColor(json.preferences.themeColor || "indigo");
        setFontSize(json.preferences.fontSize || "medium");
      }
    } catch {
  ...
  async function handleChangePassword(e) {
    ...
    if (!passwords.current || !passwords.next)
      return setMessage({
        type: "error",
        text: "Fill current and new password",
      });
    ...
    try {
      ...
      const json = await res.json();
      setMessage({ type: "success", text: json.message || "Password changed" });
  ...
  async function handleSavePreferences() {
    try {
      const url = `${baseApi}/update-preferences/${employeeId}`;
      const body = {
        twoFactor,
        notifications,
        profileVisibility,
    ...
    <div className="min-h-screen p-6 bg-white">
      <div className="max-w-6xl mx-auto space-y-6">
        <motion.h1
          initial={{ opacity: 0, y: -8 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-3xl font-bold text-indigo-600 text-center"
        >
          Settings
        </motion.h1>
