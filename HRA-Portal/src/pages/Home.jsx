...
export default function Home() {
  ...
  // Dummy state placeholders for API data
  const [employeeCount] = useState(1232);
  const [pendingLeaves] = useState(5);
  const [supportQueries] = useState([{ id: 1 }, { id: 2 }]);
  const [projects] = useState([{ id: 1 }, { id: 2 }, { id: 3 }]);
  ...
  const quickLinks = [
    ...
    { name: "Holidays", path: "/holiday", icon: Calendar },
  ];
  ...
  return (
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className="bg-white border border-gray-200 text-gray-900 p-5 rounded-xl shadow-sm
      ...
      >
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Here's a summary of your workplace insights.
          </p>
        </div>
        <img
          src={user?.avatar || "https://avatar.iran.liara.run/public"}
          alt="avatar"
          className="w-16 h-16 rounded-full mt-3 md:mt-0"
        />
      </motion.div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { title: "Pending Leaves", value: pendingLeaves, icon: Calendar },
          { title: "Active Projects", value: projects.length, icon: Briefcase },
          {
            title: "Support Queries",
            value: supportQueries.length,
            icon: ClipboardList,
          },
        ].map((item, index) => (
          <motion.div
            key={index}
            className="p-5 rounded-xl bg-white border border-gray-200 shadow-sm flex items-center gap-3"
          >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {quickLinks.map(({ name, path, icon: Icon }, idx) => (
            <Link
              key={idx}
              to={path}
              className="flex flex-col items-center justify-center p-4 rounded-xl
              bg-white border border-gray-200 shadow-sm hover:bg-gray-100 transition"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Employee Growth */}
        <motion.div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
        ...
        <motion.div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={attendanceData}>
              ...
              <Bar dataKey="leave" fill="#818cf8" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Top Employees */}
        <motion.div className="p-5 rounded-xl bg-white border border-gray-200 shadow-sm">
          <h2 className="text-base font-semibold text-gray-800 mb-3 flex items-center gap-2">
            <Award className="w-4 h-4 text-indigo-600" /> Top Performing
            Employees
          </h2>
          <ul className="divide-y">
            {topEmployees.map((emp, i) => (
              <li key={i} className="py-2 flex justify-between text-sm">
                <span className="text-gray-700">
                  {emp.name} - <span className="text-gray-500">{emp.dept}</span>
                </span>
        ...
        <motion.div className="p-5 rounded-xl bg-white border border-gray-200 shadow-sm">
          <h2 className="text-base font-semibold text-gray-800 mb-3 flex items-center gap-2">
            <Bell className="w-4 h-4 text-indigo-600" /> Announcements
          </h2>
          <ul className="divide-y">
            {announcements.map((note, i) => (
              <li key={i} className="py-2 flex justify-between text-sm">
